...
public class CwslHealerHealPadSkill : CwslPlayerSkillBase
{
    ...
    public override bool TryUseSkillSlotServer(ulong senderClientId, int slotIndex, Vector3 worldPoint)
    {
        ...
        var point = worldPoint.sqrMagnitude < 0.01f ? transform.position : worldPoint;
        point.y = 0.05f;
        point = CwslArenaUtility.ClampToPlayArea(point, 0.5f);
        CwslHealerHealPad.SpawnServer(point);
        return true;
    }
